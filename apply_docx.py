...
def set_cant_split(tr):
    """Добавляет <w:cantSplit/> в <w:trPr> строки (запрет разрыва строки между
    страницами). trPr должен быть ПЕРВЫМ ребёнком <w:tr>."""
...
def set_table_keep_rows(root):
    """Экспериментально: для каждой таблицы у первых 3 строк и предпоследней
    строки ставит keepNext ("не отрывать от следующего") на абзацы И cantSplit
    ("не разрывать строку между страницами") на саму строку. Цель — чтобы шапка
    и начало таблицы не отрывались от продолжения, а последняя строка не
    оставалась одна. Возвращает число затронутых строк.
    ВАЖНО: поведение таблиц в Word зависит от многих факторов (высота строк,
    разметка, влезает ли таблица на страницу целиком) — проверять глазами;
    ...
    for tbl in root.iter(qn('tbl')):
        ...
        if n == 0:
            continue
        # cantSplit — на ВСЕ строки (ни одна строка не разрывается между страницами)
        for tr in rows:
            set_cant_split(tr)
        # keepNext — на первые 3 и предпоследнюю (не отрывать от следующей строки)
